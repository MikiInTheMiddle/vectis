...
export default function ReviewLayer() {
  ...
  return <div className="reviewLayer">
    ...
    <button className="reviewLauncher" onClick={() => { setPanelOpen(true); setPlacing(false); }} aria-label={`${pageComments.filter((item) => !item.resolved).length} commenti aperti in questa pagina, ${totalOpen} totali`}><span>{pageComments.filter((item) => !item.resolved).length}</span> qui · <small className="reviewTotal">{totalOpen} tot</small></button>
